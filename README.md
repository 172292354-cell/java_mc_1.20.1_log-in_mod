...
**LoginMod 采用「署名 - 非商业使用 - 相同方式共享（CC BY-NC-SA 4.0 同类精神）」，即：**

| 权利 | 条件 |
|------|------|
| ✅ 个人学习、个人服务器使用 | **自由** |
| ✅ 下载、修改、二次分发 | **保留原作者署名** + **修改后的版本必须以相同协议开源**（Copyleft） |
| ❌ 用于**任何商业用途**（包括但不限于：收费入服、VIP 服务、广告变现、售卖账号权限、出售模组本体） | **严格禁止** |

**核心三条：**

1. **禁止商用** — 任何形式的商业盈利行为（含间接盈利）都不被允许。
2. **署名保留** — 无论是二次分发、公开分享、还是在自己服务器宣传中提及本模组，都必须保留原作者信息。
3. **修改后必须开源** — 如果你对本模组做了修改，并把修改后的版本放入自己的服务器 / 公开分享 / 分发给其他人，**必须把修改后的完整源码也以同样方式公开发布**（例如上传到 GitHub），让下游玩家也能看到并继续修改。

> 本项目**不是** MIT License。如果你看到 README 其他地方还写着 MIT，以本章节为准。

详细条款见项目根目录下的 [`LICENSE`](LICENSE) 文件。
